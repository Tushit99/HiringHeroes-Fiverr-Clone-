...
async function logodata() {
  try {
    let res = await fetch(`http://localhost:3000/${opt_data}`);
...
async function delme(id) {
  let res = await fetch(`http://localhost:3000/${opt_data}/${id}`, {
    method: "DELETE",
    headers: {
...
async function sortLow() {
  try {
    let res = await fetch(
      `http://localhost:3000/${opt_data}?_sort=price&_order=asc`
    );
    let data = await res.json();
    //   console.log(data);
    appendData(data);
...
async function sortHigh() {
  try {
    let res = await fetch(
      `http://localhost:3000/${opt_data}?_sort=price&_order=desc`
    );
    let data = await res.json();
    //   console.log(data);
    appendData(data);
...
async function priceless(sel) {
  try {
    let res = await fetch(`http://localhost:3000/${opt_data}?price_lte=${sel}`);
    let data = await res.json();
    //   console.log(data);
    appendData(data);
  } catch (err) {
...
async function priceMorethan(sel) {
  try {
    let res = await fetch(`http://localhost:3000/${opt_data}?price_gte=10000`);
    let data = await res.json();
    //   console.log(data);
    appendData(data);
  } catch (err) {
...
async function SaveDataType() {
  ...
  let res = await fetch(`http://localhost:3000/${Data_type}`, {
    method: "POST",
