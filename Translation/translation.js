...
document.getElementById("nav").innerHTML = navbar2(); 
...
const getAllData=async()=>{ //Getting all data for creating paginate buttons
    ...
    try{
        let res=await fetch('http://localhost:8080/translate')
...
document.getElementById("CartPg").addEventListener("click", function (){
    window.location.href = "../Cart/cart.html"
})

// ===============================

let an = localStorage.getItem("UserName");   
document.getElementById("loginPage").innerText = an ;   
...
const getData=async(page_id)=>{ // Getting 12 data per page
    ...
    try{
        let res=await fetch(`http://localhost:8080/translate?_page=${page_id}&_limit=12`)
        let data=await res.json()
...
const Sort_by_range=async(order)=>{
    ...
    try{
        let res=await fetch(`http://localhost:8080/translate?price_${order}`)
        let data=await res.json()
        console.log(data)
        appendData(data)
...
const Sort=async(order)=>{
    ...
    try{
        let res=await fetch(`http://localhost:8080/translate?_sort=price&_order=${order}`)
        let data=await res.json()
        appendData(data)
...
const Sort_by_rating=async(order)=>{
     try{
        let res=await fetch(`http://localhost:8080/translate?_sort=rating&_order=${order}`)
        let data=await res.json()
        appendData(data)
...
window.addEventListener("scroll", function(){
    if(window.pageYOffset < 96.5){
       ...
       document.querySelector("#nav").style.top = "87px"
        document.querySelector("#header1").style.backgroundColor="white";
